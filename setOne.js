...
const parameterNames = (parameter1, parameter2) => `The first parameter is ${parameter1}. The second parameter is ${parameter2}`


// INVOKE THE FUNCTION HERE. THE PARAMETERS TAKE ANY DATATYPE.

console.log(parameterNames("name1", "name2"))
...
greeting("Harrison", "Dutson", (fullName) => {console.log(`Hello, my full name is ${fullName}`)})
...
const pricesPlusTax = (pricesArr, callback) => {
    for(i = 0; i < pricesArr.length; i++){
        totalCost.push(pricesArr[i] * 1.2)
    }
...
const callTotal = function(total){
    console.log(`The new array plus tax = ${total}`)
}

pricesPlusTax(prices, callTotal)
...
function multiplyingFactory(num){
    return function(differentNum) {
        if (num >= 5){
            console.log(num * differentNum)
        } else {
